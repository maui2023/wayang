...
export function card(row) {
  ...
  const yearPlaceholder = getLang() === 'ms' ? 'Tahun —' : 'Year —';
  article.innerHTML = `<a href="details.html?id=${encodeURIComponent(row.id)}"><div class="poster-wrap"><span class="poster-type">${e(labels[row.type])}</span></div><h3>${e(row.title)}</h3><div class="card-meta"><span>${e(row.year || yearPlaceholder)} · ${e(labels[row.type])}</span><span class="rating">${row.rating == null ? '—' : `★ ${e(row.rating)}`}</span></div></a>`;
...
export function section(title, rows, href) {
  ...
  section.innerHTML = `<div class="section-head"><h2>${e(title)}</h2><a href="${e(href)}">${e(t('home.view_all'))}</a></div>`;
