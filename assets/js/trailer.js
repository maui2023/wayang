...
export function trailerButton(trailer) {
  ...
  button.textContent = validTrailer(trailer) ? t('trailer.watch') : t('trailer.unavailable'); button.disabled = !validTrailer(trailer);
  button.addEventListener('click', () => {
    ...
    if (!modal) {
      ...
      modal.innerHTML = `<div class="modal-dialog modal-dialog-centered modal-xl"><div class="modal-content"><div class="modal-header"><h2 id="trailer-heading" class="modal-title fs-5">${e(t('trailer.official'))}</h2><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="${e(t('trailer.close'))}"></button></div><div class="modal-body"><div class="ratio ratio-16x9"></div><p class="small text-secondary mt-3 mb-0">${e(t('trailer.youtube_note'))}</p></div></div></div>`;
    ...
    const frame = document.createElement('iframe'); frame.title = trailer.title || t('trailer.official');
