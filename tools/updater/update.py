...
def resolve_external_conflicts(records):
    ...
    for (kind,provider,ident),rows in owners.items():
        ...
        if provider in ('tvmaze','wikidata'): raise ValueError('Duplicate primary provider identity')
...
def main():
    ...
    parser.add_argument('--source', choices=('tvmaze','wikidata','fixture','all'), default='tvmaze')
    ...
    if args.source in ('tvmaze','wikidata','all'):
        records = []
        if args.source in ('tvmaze','all'):
            from sources.tvmaze import fetch, fetch_all
            config=json.loads(args.config.read_text())
            records = fetch_all(config,args.cache_dir/'tvmaze') if args.scope == 'full' else fetch(config)
            # Keep previously enriched curated fields only if the provider timestamp agrees.
            for path in (ROOT/'data').glob('*/*.json'):
                if path.parent.name not in TYPES: continue
                for previous in json.loads(path.read_text()):
                    if previous['id'] in {f"{item['type']}-tvmaze-{item['id']}" for item in config['shows']}:
                        for i,current in enumerate(records):
                            if current['id']==previous['id'] and current['updated_at']==previous['updated_at']:
                                ...
                                records[i]=previous;break
        elif args.source == 'wikidata':
            for path in (ROOT/'data').glob('*/*.json'):
                if path.parent.name in ('tv', 'anime'):
                    records.extend(json.loads(path.read_text()))
        if args.source in ('wikidata','all'):
            from sources.wikidata import fetch as fetch_movies
            records.extend(fetch_movies(args.cache_dir/'wikidata'))
        records, conflicts = resolve_external_conflicts(deduplicate(records))
